create or replace package body "REST_DB_LINK_API".rest_db_links
as

--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  function http_rest_request 
            ( 
              p_url          varchar2
  ...
  as
    ...
    l_clob           clob;
  begin
    --lg('p_req_method: '||p_req_method||' URL: '||p_url||' p_req_body: '||nvl(p_req_body, '**NULL**')||' p_rec_ctype: '||p_req_ctype);
    -- prepare request
    l_http_request := utl_http.begin_request (url=> p_url, method => p_req_method);
    if p_req_ctype is not null
      then
        utl_http.set_body_charset('UTF-8');  --> This row must be here to not get character conversion.
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  begin
    l_row.limit$id :=0;
    l_row.rest$url := p_url;
    while coalesce(l_hasmore, 'false') = 'true'
    loop
      l_row.limit$id := l_row.limit$id +1;
      -- Piping Row
      l_row.response := http_rest_request(p_url => l_row.rest$url,p_req_ctype => 'application/json;charset=UTF-8');
      pipe row(l_row);
      --
      select hasMore, href
        into l_hasMore
